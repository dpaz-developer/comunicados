/*CREATE TABLE contacts (
 ID_Contact mediumint(8) unsigned default null auto_increment,
...
CALL sp_set_nota ( 'nota de test', '<p>Hola este es el cuerpo de la nota  de test modificado</p>', 'Resumen de la nota de test',
1, 'active', 1, 'http://www.marti.mx/laimagen_modificada.jpg');
...
CALL sp_get_total_notices (1, 'active', 'razones');
...
call sp_update_status_notice(1,'active');
...
BEGIN
	SELECT id, title, body, summary, sectionId, dateRegistration, status, userLoadId, urlImageMain
	FROM notices
	WHERE status=v_status AND sectionId=v_seccionId AND title LIKE CONCAT('%', v_query,'%')
	ORDER BY dateRegistration DESC, id DESC
	LIMIT v_offset,v_tamanioPagina;
END//
...
CALL sp_get_search_notice ('', 3, 'active', 10, 3);

SELECT id, dateRegistration, title, sectionId
FROM notices
WHERE status = 'active'
ORDER BY  dateRegistration DESC, id DESC LIMIT 3,10;
  -- limit offset, max resultados

SELECT dateRegistration, title, id
FROM notices
-- WHERE MATCH(title) AGAINST('Razones de peso')
ORDER BY dateRegistration DESC;

-- decubrimos una manera mejor
SELECT id, dateRegistration, title, sectionId 
FROM notices 
WHERE id IN (
	SELECT id FROM notices ORDER BY dateRegistration DESC
)
LIMIT 1, 10;
...
-- vamos a crear el script para el detalle de la nota
